			...
			if(_isGarrison) then {
				...
				if(_spawnType == 1) then {
					...
					for "_i" from 1 to _numberOfUnits do {
						private _group = [getPos _logic, _factionComp, _type, _radius, getMarkerPos _target] call FU_fnc_spawnAttackers;
						sleep 1;
					};
				} else {
					...
					for "_i" from 1 to _numberOfUnits do {
						private _group = [getPos _logic, _factionComp, _type, _radius, _patrolRadius] call FU_fnc_spawnDefenders;
						sleep 1;
					};
				};
			};

			FU_stateMachine = FU_stateMachine - [[[_logic, _spawnType], _factionComp, _isGarrison]];
