...
if(missionNamespace getVariable "FU_BlizzardDifficulty" >= 0) then {
	[] spawn FU_fnc_blizzardEffect;
	[] spawn FU_fnc_blizzardLogic;
	if(missionNamespace getVariable "FU_BlizzardMaskOverlay") then {[] spawn FU_fnc_blizzardMask;};
};

if(missionNamespace getVariable "FU_BlizzardDifficulty" >= 1) then {
	[] spawn FU_fnc_blizzardUI;
	if(missionNamespace getVariable "FU_BlizzardAllowTearing") then {[] spawn FU_fnc_blizzardTearing;};
};

if(missionNamespace getVariable "FU_BlizzardDifficulty" >= 2) then {
	[] spawn FU_fnc_blizzardFrost;
	[] spawn FU_fnc_blizzardHazard;
};
